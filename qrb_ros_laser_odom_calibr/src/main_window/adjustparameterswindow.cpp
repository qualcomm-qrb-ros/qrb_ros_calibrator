...
AdjustParametersWindow::AdjustParametersWindow(QWidget * parent,
    std::shared_ptr<qrb::laser_odom_calibrator::Calibrator> calibrator)
  : QMainWindow(parent), calibrator_(calibrator), ui_(new Ui::AdjustParametersWindow)
{
  ui_->setupUi(this);
  calibrator_->get_parameters_adjust(
      mid_max_dist_seen_as_continuous_, mid_line_length_tolerance_, mid_ransac_fitline_dist_th_);
  current_frame_id_ = 0;
  min_size_ = 0;
  ...
  for (int i = 0; i < calibrator_->laser_data_set.size(); ++i) {
    calibrator_->update_parameters_detect(i, mid_max_dist_seen_as_continuous_,
        mid_line_length_tolerance_, mid_ransac_fitline_dist_th_);
    progress.setValue(i);
  }
  ...
  ui_->p2Slider->setRange(0, 200);
  ui_->p2Slider->setValue(100);
  ui_->p2_value->setText(QString::fromStdString(std::to_string(mid_line_length_tolerance_)));
...
void AdjustParametersWindow::on_p2Slider_valueChanged(int value)
{
  line_length_tolerance_ = (mid_line_length_tolerance_ / 100) * double(value);
  ui_->p2_value->setText(QString::fromStdString(std::to_string(line_length_tolerance_)));
}
...
{
  calibrator_->update_parameters_detect(current_frame_id_, max_dist_seen_as_continuous_,
      line_length_tolerance_, ransac_fitline_dist_th_);
  viewer_->updata_laser_data(calibrator_->laser_data_set[current_frame_id_]);
}

void AdjustParametersWindow::on_p2Slider_sliderReleased()
{
  calibrator_->update_parameters_detect(current_frame_id_, max_dist_seen_as_continuous_,
      line_length_tolerance_, ransac_fitline_dist_th_);
  viewer_->updata_laser_data(calibrator_->laser_data_set[current_frame_id_]);
...
void AdjustParametersWindow::on_p2Slider_sliderPressed()
{
  double value = double(ui_->p2Slider->value());
  line_length_tolerance_ = (line_length_tolerance_ / 100.0) * value;
  calibrator_->update_parameters_detect(current_frame_id_, max_dist_seen_as_continuous_,
      line_length_tolerance_, ransac_fitline_dist_th_);
  viewer_->updata_laser_data(calibrator_->laser_data_set[current_frame_id_]);
}
