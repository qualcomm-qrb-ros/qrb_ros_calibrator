...
void PointCloudViewer::updata_laser_data(const qrb::laser_odom_calibrator::LaserData & laser_data)
{
  ...
  intersaction.x = laser_data.intersaction_point(0);
  intersaction.y = laser_data.intersaction_point(1);
  intersaction.z = 0.0;
  pcl::PointXYZ short_end;
  short_end.x = laser_data.short_end_pt.x;
  short_end.y = laser_data.short_end_pt.y;
  short_end.z = 0.0;
  pcl::PointXYZ long_end;
  long_end.x = laser_data.long_end_pt.x;
  long_end.y = laser_data.long_end_pt.y;
  long_end.z = 0.0;
  if (laser_data.can_be_used != false) {
    draw_line(intersaction, short_end);
    draw_line(intersaction, long_end);
  }
}
